...
class LaunchEntry(Gtk.Box):
    ...
    def _make_widgets(self):
        ...
        description_box, self._description_entry = compose_entry_box("Description", "Unspecified")
        executable_box, self._executable_entry = compose_entry_box("Executable", "Unspecified", False, "folder")
        workdir_box, self._workdir_entry = compose_entry_box("Working Directory", "Unspecified", False, "folder")
        arguments_box, self._arguments_entry = compose_entry_box("Launch Arguments", "Unspecified")
        delete_button = Gtk.Button()
        bottom_box = Gtk.Box()
        checkbutton_box = Gtk.Box(hexpand=True)
        self._windows_checkbutton = Gtk.CheckButton(label="Windows")
        self._mac_checkbutton = Gtk.CheckButton(label="Mac")
        self._linux_checkbutton = Gtk.CheckButton(label="Linux")
        ...
        checkbutton_box.append(self._windows_checkbutton)
        checkbutton_box.append(self._mac_checkbutton)
        checkbutton_box.append(self._linux_checkbutton)
    ...
    def _configure_widgets(self):
        self.set_spacing(15)
        self._description_entry.set_text(self._entry.get("description", ""))
        self._executable_entry.set_text(self._entry.get("executable", ""))
        self._workdir_entry.set_text(self._entry.get("workingdir", ""))
        arguments = self._entry.get("arguments", "")
        if isinstance(arguments, int): arguments = ""
        self._arguments_entry.set_text(arguments)

        entry_oslist = self._entry.get("config", {}).get("oslist", "").lower()
        self._windows_checkbutton.set_active("windows" in entry_oslist)
        self._mac_checkbutton.set_active("macos" in entry_oslist)
        self._linux_checkbutton.set_active("linux" in entry_oslist)
    ...
    def _make_oslist_string(self) -> str:
        selected_os = []
        if self._windows_checkbutton.get_active():
            selected_os.append("windows")
        if self._mac_checkbutton.get_active():
            selected_os.append("macos")
        if self._linux_checkbutton.get_active():
            selected_os.append("linux")
        return ','.join(selected_os)

    def get_updated_entry(self) -> dict:
        if self._description_entry.get_text():
            self._entry["description"] = self._description_entry.get_text()
        if self._executable_entry.get_text():
            self._entry["executable"] = self._executable_entry.get_text()
        if self._workdir_entry.get_text():
            self._entry["workingdir"] = self._workdir_entry.get_text()
        if self._arguments_entry.get_text():
            self._entry["arguments"] = self._arguments_entry.get_text()
        oslist = self._make_oslist_string()
        if oslist:
            self._entry.setdefault("config", {})
            self._entry["config"]["oslist"] = oslist
        return self._entry
